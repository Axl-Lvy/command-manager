...
class OrderAListView extends VerticalLayout {
  ...
  OrderAListView(OrderAService orderAService) {
    this.orderAService = orderAService;

    var addBtn =
        new Button("Nouvelle commande", event -> Notification.show("TODO: formulaire commande A"));
    addBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY);

    grid = new Grid<>();
    grid.addColumn(OrderA::getOrderNumber).setHeader("N° Commande").setAutoWidth(true);
    grid.addColumn(order -> Optional.ofNullable(order.getClient()).map(c -> c.getName()).orElse(""))
        .setHeader("Client")
        .setFlexGrow(1);
